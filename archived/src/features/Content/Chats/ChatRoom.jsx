...
const ChatRoom = () => {
  ...
  useEffect(() => {
    // Skip Socket.IO in static mode
    const isStaticMode = import.meta.env.VITE_STATIC_MODE === 'true' || import.meta.env.MODE === 'static';
    if (isStaticMode) {
      setIsConnected(true); // Pretend we're connected in static mode
      return; // Don't initialize Socket.IO in static mode
    }
  ...
  useEffect(() => {
    const loadProfile = async () => {
      try {
        ...
        if (!isPreparationChat && chatId && (chatId.startsWith('chat_') || chatId.startsWith('chat-'))) {
