...
function Menu() {
  ...
  const logout = async () => {
    ...
    try {
      if (isStaticMode) {
        // In static mode, set logout flag and navigate
        sessionStorage.setItem('loggedOut', 'true');
        // Force immediate navigation - use hash for HashRouter
      ...
      // Even if logout fails, navigate to login in static mode
      if (isStaticMode) {
        navigate("/");
        window.location.reload();
      }
  ...
  useEffect(() => {
    // Get current user ID from cookies or use static mode user ID
    const isStaticMode = import.meta.env.VITE_STATIC_MODE === 'true' || import.meta.env.MODE === 'static';
    ...
    if (isStaticMode) {
      ...
      // In dynamic mode, get from cookies
  ...
  useEffect(() => {
    ...
    if (isStaticMode) {
      // In static mode, listen for custom events
      const handleStaticNotification = () => {
